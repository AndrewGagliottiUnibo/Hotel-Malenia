...
public class RestaurantPages {
	...
	breakfast.addActionListener(e -> {
	    ResultSet tables = this.logic.showBreakfastTables();
	    this.printTextArea(tables);
	});
	...
	lunch.addActionListener(e -> {
	    ResultSet tables = this.logic.showLaunchTables();
	    this.printTextArea(tables);
	});
	...
	dinner.addActionListener(e -> {
	    ResultSet tables = this.logic.showDinnerTables();
	    this.printTextArea(tables);
	});
	...
	/*
	 * TextArea to be updated when you require a particular set of clients to be
	 * shown.
	 */
     ...
     * @param tables
     */
    private void printTextArea(final ResultSet tables) {
	this.textArea.setText("");
	try {
	    while (tables.next()) {
		this.textArea.append(tables.getString(1) + "\n");
	    }
